...
AssigneeData::Type::Type(const std::string& name_, bool isDefaultConstructible_) noexcept
: name(name_), isDefaultConstructible(isDefaultConstructible_) {
}

AssigneeData::Builder::Builder(AssigneeData&& assignee) : _assignee(std::move(assignee)) {
}

AssigneeData::Builder& AssigneeData::Builder::name(const llvm::StringRef& name) {
  _assignee.name = name.rtrim();
  return *this;
}

AssigneeData::Builder&
AssigneeData::Builder::type(const llvm::StringRef& type, bool isDefaultConstructible) {
...
std::string AssigneeData::toString(bool withType) const {
  if (withType) {
    assert(type.has_value() && "Requested assignee string with type, but have no type");
