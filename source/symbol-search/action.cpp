...
bool Action::BeginSourceFileAction(clang::CompilerInstance& compiler,
  ...
  const auto location = sourceManager.translateLineCol(fileID, _line, _column);
...
clang::FileID Action::_getFileID(clang::SourceManager& sourceManager) const {
  ...
  const auto* fileEntry = fileManager.getFile(_filename);
  if (fileEntry == nullptr || !fileEntry->isValid()) {
    llvm::errs() << "Could not find file " << _filename << " in file manager\n";
    std::exit(EXIT_FAILURE);
  }

  assert(fileEntry->getName() == _filename);
